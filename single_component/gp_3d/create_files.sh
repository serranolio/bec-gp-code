...
ramp_times=($(seq -f "%.0f" 10 10 10))

# Files to copy
files=(execute_simulation.py parameters.py system_functions.py)

for t in "${ramp_times[@]}"; do
    (
        dir_name="simulation_delta_i_${detuning_start}_delta_f_${detuning_end}_omega_l_i_${lattice_strength_start}_omega_l_f_${lattice_strength_end}_ramp_time_${t}ms"
        mkdir -p "$dir_name"
