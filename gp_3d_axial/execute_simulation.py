...
out_file_name = (directory_path + 
                 '/output/polarization_' + sample_str + params_str)
...
3. TWA-noise ----------------------------------------------------------
'''
noise = (np.random.normal(scale=1/np.sqrt(4*n_atoms*np.repeat(dv, 3)), 
                          size=3*nx*nz) +
         1j*np.random.normal(scale=1/np.sqrt(4*n_atoms*np.repeat(dv, 3)), 
                             size=3*nx*nz))
...
'''
print('saving data')
n = ((psi.conj()*psi).real.reshape((3, nx*nz, t_frames)) * 
     dv[None, :, None]).sum(axis=1)

polarization = (n[0] - n[1]) / (n[0] + n[1])

np.save(out_file_name, polarization)
