...
def split_step_imag_time(psi0, 
    ...
    for _ in range(steps):
        ...
        if normalize_each_step:
            norm = dz * (np.abs(psi0)**2 + np.abs(psi1)**2).sum()
            psi0 /= np.sqrt(norm)
            psi1 /= np.sqrt(norm)
...
def sequence_cooling(t, psi, delta, omega):
    norm = np.sum(dz*np.abs(psi.reshape((2, -1)))**2)
...
def get_ground_state(steps, step_size, delta, omega):
    ...
    return np.array(psi.reshape(2*nz))
